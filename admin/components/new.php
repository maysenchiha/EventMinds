...
<html lang="en">
  ...
  <body>
                  ...
                  <div class="table-responsive">
                    <table id="multi-filter-select" class="display table table-striped table-hover">
                        ...
                        <tr>
                            ...
                            <th scope="col" class="text-center">Status</th> <!-- Nouvelle colonne ajoutée -->
                        </tr>
                        </thead>
                        <tbody>
                            <?php include '../../images.php';
                                if (!empty($completcc)) {
                                    foreach ($completcc as $event) {
                                        $eventDateTime = htmlspecialchars($event['datetime']);
                                        $eventLieu = htmlspecialchars($event['lieu']);
                                        $eventType = htmlspecialchars($event['type']);
                                        $eventId = $event['id']; // ID de l'événement pour la mise à jour
                            ?>
                            <tr id="event-<?= $eventId ?>">
                                ...
                                <td class="text-center">
                                    <div class="form-button-action">
                                     <button id="approveButton_<?= $eventId ?>" class="badge badge-success me-2" onclick="approveEvent(<?= $eventId ?>)">Approve</button>
                                     <button id="rejectButton_<?= $eventId ?>" class="badge badge-danger" onclick="rejectEvent(<?= $eventId ?>)">Reject</button>
                                    </div>
                                </td>
                            </tr>
                            <?php
    ...
    // Créer un nouvel objet XMLHttpRequest pour la requête AJAX
    ...
    // Définir la fonction de rappel pour le traitement de la réponse
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
            // Affiche la réponse du serveur dans la console
            console.log(xhr.responseText);  // Ajoute cette ligne pour voir ce que le serveur envoie
            ...
            try {
                ...
                if (response.status === 'approved') {
                    // Masquer uniquement le bouton "Approve" une fois l'événement approuvé
                    document.getElementById('approveButton_' + eventId).style.display = 'none';
                    document.getElementById('rejectButton_' + eventId).style.display = 'inline-block';
                    // Le bouton "Reject" reste visible (il n'est pas modifié ici)
                }
            } catch (e) {
    ...
    xhr.send("eventId=" + eventId + "&status=approved");
}
...
function rejectEvent(eventId) {
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
            // Affiche la réponse du serveur dans la console
            console.log(xhr.responseText);  // Ajoute cette ligne pour voir ce que le serveur renvoie
            ...
            try {
                ...
                if (response.status === 'rejected') {
                    // Masquer le bouton "Reject" et afficher le bouton "Approve"
                    document.getElementById('rejectButton_' + eventId).style.display = 'none';
                    document.getElementById('approveButton_' + eventId).style.display = 'inline-block';  // Afficher le bouton "Approve"
                }
            } catch (e) {
                console.error('Erreur lors du parsing JSON: ', e);
            }
    ...
    // Envoi de la requête avec les données nécessaires
    xhr.send("eventId=" + eventId + "&status=rejected");
};
